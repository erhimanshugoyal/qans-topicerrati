...
//	$('#user_interest').html(interest)
   ...
   'click .follow': function () {
	...
	following = []
			...
			if (already_nof == null){
        		Notification.insert({u_id:q_id,follower_id:Meteor.userId(),is_seen:0,is_unfollowed:0,is_topic:0,created_at:new Date(),is_userfollow:1,is_f_notify:1})
			}else{
...
Template.topic.events({
	...
	already_nof = Notification.findOne({u_id:user_id,follower_id:Meteor.userId(),is_topic:1,is_unfollowed:1,is_f_notify:1})
		if (already_nof == null){
		Notification.insert({u_id:user_id,follower_id:Meteor.userId(),is_seen:0,is_unfollowed:0,is_topic:1,topic_id:q_id,created_at:new Date(),is_f_notify:1})
		}else{
		Notification.update(already_nof._id,{$set:{is_unfollowed:0}})
		}
         div_id = '#follow' + q_id;
...
//	var answer = $(div_id).html().replace(/contenteditable=\"true\"/g,'')
	...
	Notification.insert({u_id:q_user,follower_id:Meteor.userId(),is_seen:0,is_unfollowed:0,is_topic:0,is_answer:1,created_at:new Date(),is_f_notify:0})
	});
...
//      alert(answer);
	...
	Notification.insert({u_id:q_uid,follower_id:Meteor.userId(),is_seen:0,is_unfollowed:0,is_topic:0,is_comment:1,created_at:new Date(),is_f_notify:0})
        });
...
//      alert(answer);
	...
	Notification.insert({u_id:q_uid,follower_id:Meteor.userId(),is_seen:0,is_unfollowed:0,is_topic:0,is_reply:1,created_at:new Date(),is_f_notify:0})
        });
...
*/
  Template.everyq.events({
	...
	'click .answer_edit': function () {
                div_id = '#anser' + this.question_id;
                $(div_id).show();
                div_id = '#answer_edit' + this.question_id;
